...
/**
 * [RecyclerView.ItemDecoration] that sticks header cell in top of the list
 * inspired from https://gist.github.com/filipkowicz/1a769001fae407b8813ab4387c42fcbd
 */
class HeaderItemDecoration(
    ...
    init {
        ...
        val gestureDetector = GestureDetector(parent.context, object : SimpleOnGestureListener() {
        ...
        parent.addOnItemTouchListener(object : RecyclerView.SimpleOnItemTouchListener() {
            override fun onInterceptTouchEvent(
                ...
                //looking for real bottom position of header as it may have been moved up
                var currentHeaderBottom = currentHeader?.second?.itemView?.bottom ?: 0
                val contactPoint = currentHeaderBottom + parent.paddingTop
                val childInContact = getChildInContact(parent, contactPoint)
                if (childInContact != null && isHeader(parent.getChildAdapterPosition(childInContact))) {
                    currentHeaderBottom = childInContact.top
                }

                if (motionEvent.y <= currentHeaderBottom && gestureDetector.onTouchEvent(motionEvent)) {
                    currentHeader?.let {
                        if (headerClickListener.onHeaderClick(it.first) == true) {
                            // if a sticky header was just collapsed, keep it in top of the screen
                            parent.layoutManager?.scrollToPosition(it.first)
                        }
                        return true
                    }
                }
